...
export function DoctorReportModal() {
  ...
  const handleCopyMarkdown = async () => {
    ...
    // a LAN IP, which is exactly how a clinic would reach a self-hosted instance --
    // and `writeText` returns a Promise that rejects when permission is denied or
    // the document is not focused. The previous three lines called it unguarded,
    // un-awaited and uncaught, so on those deployments the click threw a
    // TypeError into the console while the button cheerfully switched to
    // "Copied" and the clipboard stayed empty. Every failure path now surfaces the
    // report text instead, so the user can still get it to their clinician.
    const clipboard = typeof navigator !== 'undefined' ? navigator.clipboard : undefined;
    if (!clipboard?.writeText) {
      setCopyError(
        'Your browser blocked clipboard access (this usually means the page is not served over HTTPS). Use "Print / Save PDF" instead, or select the report text below and copy it manually.'
      );
      return;
    }

    try {
      await clipboard.writeText(reportMd);
      setCopyError(null);
      setCopied(true);
      ...
      setCopied(false);
      setCopyError(
        'Could not copy the report to your clipboard. Use "Print / Save PDF" instead, or select the report text below and copy it manually.'
      );
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-[var(--bg-surface)] text-[var(--text-primary)]">
          {/* Clipboard failure notice. role="alert" so the reason is announced
              rather than only rendered -- a silent no-op is what made the old
              unguarded writeText call so hard to notice. `no-print` keeps a
              transient browser-permission message out of the printed brief. */}
          {copyError && (
            <div className="alert-warning text-xs flex items-start gap-2 no-print" role="alert">
